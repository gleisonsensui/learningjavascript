...
function calculate() {
  ...
  let valor = document.getElementById("valor-emprestimo");
  let taxaJuros = document.getElementById("juros-anual");
  let prazo = document.getElementById("prazo-pagamento");
  let cep = document.getElementById("codigo-postal");
  let parcela = document.getElementById("parcela-mensal");
  let totalEmprestimo = document.getElementById("total-financiado");
  let totalJuros = document.getElementById("total-juros");
  // Obtém a entrada do usuário através dos elementos de entrada. Presume que tudo isso é válido.

  const entries = [valor.value, taxaJuros.value, prazo.value, cep.value];

  const labels = document.querySelectorAll(".label");

  // Valida se a entrada de dados está vazio ou undefined
  entries.forEach((item, index) => {
    if (item === undefined || item === "") {
      labels.forEach((itemLabel, indLabel) => {
        if (index === indLabel) {
          alert(`O campo: ${itemLabel.textContent} não está preenchido`);
        }
      });
    }
  });

  // Converte os juros de porcentagem para decimais e converte de taxa anual para taxa mensal. Converte o período de pagamento em anos para o número de pagamentos mensais.
  let valorFinanciado = Number(valor.value);
  let jurosMensal = Number(taxaJuros.value) / 100 / 12;
  let totalMeses = Number(prazo.value) * 12;
  // Agora calcula o valor do pagamento mensal.
  let x = Math.pow(1 + jurosMensal, totalMeses); // Math.pow() calcula potências
  let parcelaMensal = (valorFinanciado * x * jurosMensal) / (x - 1);
  // Se o resultado é um número finito, a entrada do usuário estava correta e temos resultados significativos para exibir
  if (isFinite(parcelaMensal)) {
    // Preenche os campos de saída, arredondando para 2 casas decimais
    parcela.innerHTML = parcelaMensal.toFixed(2);
    totalEmprestimo.innerHTML = (parcelaMensal * totalMeses).toFixed(2);
    totalJuros.innerHTML = (
      parcelaMensal * totalMeses -
      valorFinanciado
    ).toFixed(2);

    // Salva a entrada do usuário para que possamos recuperá-la na próxima vez que ele visitar
    save(valor.value, taxaJuros.value, prazo.value, cep.value);
    ...
    try {
      ...
      getLenders(valor.value, taxaJuros.value, prazo.value, cep.value);
    } catch (e) {
      /* E ignora esses erros */
    }
    // Por fim, traça o gráfico do saldo devedor, dos juros e dos pagamentos do capital;

    chart(valorFinanciado, jurosMensal, totalMeses, parcelaMensal);
  } else {
    /*
     ...
     * */
    parcelaMensal.innerHTML = ""; // Apaga o conteúdo desses elementos
    totalEmprestimo.innerHTML = "";
    totalJuros.innerHTML = "";
    chart();
 ...
 *  Esse recurso de armazenamento não vai funcionar em alguns navegadores (o Firefox, por exemplo), se você executar o exemplo a partir de 
 *  um arquivo local:// URL. Contudo, funciona com HTTP. 
*/
function save(valor, taxaJuros, prazo, cep) {
  if (window.localStorage) {
    // Só faz isso se o navegador suportar
    localStorage.loan_amount = valor;
    localStorage.loan_apr = taxaJuros;
    localStorage.loan_years = prazo;
    localStorage.loan_zipcode = cep;
  }
}
...
window.onload = function () {
  ...
  if (window.localStorage && localStorage.loan_amount) {
    document.getElementById("valor-emprestimo").value = localStorage.loan_amount;
    document.getElementById("juros-anual").value = localStorage.loan_apr;
    document.getElementById("prazo-pagamento").value = localStorage.loan_years;
    document.getElementById("codigo-postal").value = localStorage.loan_zipcode;
  }
};
